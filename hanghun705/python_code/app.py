...
import threading
from model import SignLanguageTranslator, TranslationResult, VideoInfo, FrameKeypoints # model.py 에서 필요한 클래스 import
...
def load_dictionary_data():
    ...
    for i in range(1, 51):
        ...
        if not os.path.exists(json_filepath):
            logging.warning(f"Dictionary Load: JSON file not found for ID {file_id}: {json_filepath}")
            continue
        
        if not os.path.exists(video_filepath):
            logging.warning(f"Dictionary Load: Video file not found for ID {file_id}: {video_filepath}")
        ...
        try:
            ...
            logging.error(f"Error processing JSON file {json_filepath}: {e}")

    logging.info(f"Loaded {len(dictionary)} dictionary entries from database.")
    return dictionary

# 전역 변수에 실제 데이터 로드
DICTIONARY_DATA = load_dictionary_data()
    
# Flask 앱 인스턴스 생성 및 데이터 로드
app = Flask(
    __name__, 
...
load_dictionary_data() # 서버 시작 시 사전 데이터 로드
...
def update_task_status(file_id, progress, message):
    if file_id in TASK_STATUS:
        ...
        logging.warning(f"Task ID {file_id}를 찾을 수 없습니다. 상태 업데이트 실패.")
    ...
    if video_type == 'dictionary':
        ...
        for item in DICTIONARY_DATA:
            if item['id'] == identifier:
                ...
                logging.info(f"이 단어의 비디오 경로는 dictionary_path: {filepath}")
                return filepath
        return None
        ...
        if video_type == 'original': # 원본 영상을 찾는 경우
            logging.info(f"이 작업의 비디오 경로는 original_path: {task_data.get('original_path')}")
            return task_data.get('original_path')
        elif video_type == 'annotated': # annotated 영상을 찾는 경우
            logging.info(f"이 작업의 비디오 경로는 annotated_path: {task_data.get('annotated_path')}")
